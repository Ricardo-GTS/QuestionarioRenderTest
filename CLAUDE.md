...
**Matricula da Universidade (`User.registration_number`):** obrigatoria no cadastro e editavel em "Conta", unica, so' digitos (8 a 12, `REGISTRATION_NUMBER_VALIDATOR` em `accounts/models.py`). A coluna e' `null=True` so' por causa de contas criadas pelo login com Google e contas anteriores ao campo: `accounts.middleware.RequireRegistrationNumberMiddleware` redireciona qualquer usuario logado sem matricula para `/conta/matricula` (HTMX recebe `HX-Redirect`) ate ele preencher — liberados so' essa tela, logout, `/health`, `/django-admin/` e estaticos. Em testes, usuario criado via ORM que precisa navegar pela app tem que receber `registration_number`, senao toda request vira 302.

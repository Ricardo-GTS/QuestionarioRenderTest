...
└── services/
    ...
    ├── moderation.py       # reportes + regra de flag (should_flag) + list_questions_with_pending_reports (fila de moderacao) + resolve_reported_question (Aprovar/Rejeitar Remocao, decisao unica por pergunta) + approve_question (reativar) + update_question
...
├── style.py               # sistema de design (design.md): cores (INK/PAPER/ACCENT/TRUE_COLOR/FALSE_COLOR), fontes (Lora/IBM Plex Sans via Google Fonts), APP_THEME (radius="none", accent_color="amber" -- via rx.plugins.RadixThemesPlugin em rxconfig.py, nao rx.App(theme=...), que esta deprecated), GLOBAL_STYLE (fonte/cor padrao de rx.heading/rx.text, app-wide), e helpers (card_style, chip_style, square_button_color_props, report_status_chip_props, status_label) reusados nas paginas
├── api_client.py          # wrapper httpx, injeta Authorization no header (inclui admin_* )
├── models.py              # TypedDict tipados (AdminQuestionItem com reports: list[AdminReportItem] aninhado) -- ver armadilha do rx.foreach abaixo
├── state/                 # AuthState (+is_admin), QuestionState, QuizState, ReportState (+ REASON_CATEGORIES fixo), StatsState, admin_state.py (5 states: Moderation/Removed/Users/Settings/Stats)
├── pages/                 # login, register, home, quiz, account, estatisticas (reputacao do proprio usuario), admin_dashboard/moderation/removed/users/settings
...
**Moderacao (RF04):** `POST /questions/{id}/report` grava o `Report` (um por par question_id+reporter_id -- unique constraint, 409 se repetir) e `services/moderation.register_report` verifica a contagem; ao atingir `REPORT_THRESHOLD`, muda `Question.status` para `reported` -- isso so' controla se a pergunta sai do pool do quiz (`services/quiz.pick_random_questions` so seleciona `status == active`), **nao** controla o que o admin ve na fila de moderacao (ver abaixo).

`ReportCreate` (schemas/report.py) exige `reason_category` (um dos `REASON_CATEGORIES`) e so exige `reason` (texto livre) quando `reason_category == "Outro"` -- validado via `@model_validator(mode="after")`, nao em campos separados. `Report.reason` e' `nullable=True` no banco por causa disso.

A fila de moderacao (`GET /admin/questions/pending-reports`, `services/moderation.list_questions_with_pending_reports`) lista perguntas com **pelo menos 1 `Report.status == pending`**, independente do status da pergunta -- de propósito diferente de `status == reported`, pra um reporte nunca ficar "preso" inacessivel so' porque a pergunta ja saiu desse status (ex: foi removida antes do reporte ser resolvido -- esse era um bug real, corrigido). Tambem mostra reportes desde o primeiro, antes do auto-flag por `REPORT_THRESHOLD`.

O admin resolve uma pergunta reportada com UMA decisao que cobre a pergunta e TODOS os reportes pendentes dela de uma vez (`services/moderation.resolve_reported_question`, via `PUT /admin/questions/{id}/approve-removal` ou `/reject-removal`) -- nao ha mais decisao por reporte individual:
...
A tela de moderacao mostra quem criou a pergunta (`AdminQuestionOut.author_name`) e quem reportou (nome + motivo), mas nao tem edicao inline -- editar so existe na aba "Perguntas Removidas" (`update_question`, recalcula embedding se o enunciado mudar). Uma pergunta ja removida pode ser reativada e editada na aba admin "Perguntas Removidas" (`GET /admin/questions?status=removed`, `PUT /admin/questions/{id}/approve` -- reativa sem mexer em reportes, que ja foram resolvidos quando a pergunta foi removida). Reportes aceitos/rejeitados e perguntas removidas alimentam a reputacao de quem reportou/criou, calculada na hora (`services/stats.compute_user_reputation`), exposta em `GET /stats/me` (pagina "Estatisticas" do aluno) e em `GET /admin/users` (visao do admin).
...
**Reflex — armadilha conhecida:** esta versao do Reflex (`0.9.10.post2`) **nao gera setters automaticos** (`set_<campo>`) para vars de state simples — cada campo de formulario precisa de um metodo `set_<campo>` explicito na classe de State (ver `AuthState`, `QuestionState`, `ReportState`). Tambem, `rx.foreach` nao funciona sobre uma var `dict`/`Any` (ex: indexar um dict generico) — precisa de uma var `list[...]` com tipo concreto (ver `QuizState.feedback` como separado de um `result: dict` generico). Pra **lista aninhada** (uma lista dentro de outra, ex: reportes de cada pergunta), use `typing.TypedDict` (ver `questionario/models.py`, `AdminQuestionItem["reports"]: list[AdminReportItem]`), **nao** `rx.PropsBase` -- `PropsBase` compila certo (o `rx.foreach` aninhado funciona no JSX) mas **nao serializa direito quando atribuido dinamicamente num event handler** (`self.questions = [...]` dentro de um metodo async, o caso real de sempre): o delta mandado pro navegador carrega o objeto Python cru em vez dos campos, e tudo aparece como `undefined` na tela. Isso so aparece em runtime, nao no `reflex export` -- ja caimos nesse bug uma vez (usamos `PropsBase`, pareceu funcionar porque só testamos compilação, foi pra producao com "undefined" em tela). `TypedDict` e' um `dict` de verdade em runtime (serializa normal) com tipo concreto o suficiente pro `rx.foreach` aninhar -- os dois lados testados: `state.get_delta()` com atribuicao dinamica (serializacao) e `reflex export` com `rx.foreach` aninhado (compilacao). Tambem, `rx.theme(..., color_mode=...)` (forcar aparencia clara/escura so' num sub-componente) **nao funciona** nesta versao -- o prop de aparencia e' silenciosamente descartado na compilacao (confirmado inspecionando o JSX gerado: o `<Theme>` compilado nunca recebe `appearance`, so' `css`). Pra forcar aparencia num sub-componente, aplique a classe do Radix direto via `class_name="radix-themes light light-theme"` (ou `dark`/`dark-theme`) no componente raiz do sub-componente -- e' assim que o Radix decide as variaveis de cor (`--gray-12` etc.), ver `components/report_modal.py`. Tambem: um helper de estilo Python que decide cor/borda a partir de um valor de State (ex: `status` de um `Report`/`Question`, ou qual botao V/F esta selecionado) **nao pode fazer `if/else` em Python** sobre esse valor -- ele so existe em runtime no browser. Precisa devolver as props via `rx.cond(...)` (ver `style.report_status_chip_props`/`square_button_color_props`, que recebem a Var e devolvem um dict com `rx.cond` dentro, nunca um dict escolhido por `if kind == "true"`). Nota: `rx.App(theme=...)` esta deprecated nesta versao (warning na compilacao) -- o tema vai em `rx.plugins.RadixThemesPlugin(theme=...)` na lista `plugins` do `rxconfig.py` (ver `style.APP_THEME`), nao em `questionario.py`.
